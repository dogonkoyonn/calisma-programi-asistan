...
// Type tanımları
type LearningProfile = 'tyt-ayt' | 'software' | 'language' | 'university' | 'other';
type SkillLevel = 'beginner' | 'intermediate' | 'advanced';
type ResourceType = 'youtube' | 'pdf' | 'course' | 'quiz';
...
interface ProgramData extends UserProfile {
    schedule: WeekSchedule[];
    tasks: Task[];
    createdAt: Date;
}

// Yeni Program Yönetimi için interface'ler
...
class ResourceDatabase {
    ...
    private initializeResources(): void {
        // TYT-AYT Matematik
        this.addResources('matematik', [
            {
                title: 'Tonguç Akademi - TYT Matematik',
                type: 'youtube',
                url: 'https://youtube.com/@tongucakademi',
                description: 'Temel matematik konuları ve pratik çözümler',
                level: 'beginner',
                tags: ['matematik', 'tyt']
            },
            {
                title: 'Rehber Matematik',
                type: 'youtube',
                url: 'https://youtube.com/@rehbermatematik',
                description: 'Orta seviye matematik videoları',
                level: 'intermediate',
                tags: ['matematik', 'tyt']
            },
            {
                title: '3D Eyüp - İleri Matematik',
                type: 'youtube',
                url: 'https://youtube.com/@3deyup',
                description: 'Zor sorular ve ileri seviye teknikler',
                level: 'advanced',
                tags: ['matematik', 'tyt', 'ayt']
            }
        ]);

        // Fizik
        this.addResources('fizik', [
            {
                title: 'Fizik Adam',
                type: 'youtube',
                url: 'https://youtube.com/@fizikadam',
                description: 'Anlaşılır fizik anlatımları',
                level: 'beginner',
                tags: ['fizik', 'tyt']
            }
        ]);
        ...
        this.addResources('javascript', [
            ...
            },
            {
                ...
                description: 'İleri seviye web teknolojileri',
                level: 'advanced',
                tags: ['javascript', 'web']
            }
        ]);
        ...
        // Python
        this.addResources('python', [
            {
                title: 'Corey Schafer - Python',
                type: 'youtube',
                url: 'https://youtube.com/@coreyms',
                description: 'Python temelleri ve ileri konular',
                level: 'beginner',
                tags: ['python']
            }
        ]);
        ...
        // İngilizce
        this.addResources('ingilizce', [
            {
                title: 'English with Lucy',
                type: 'youtube',
                url: 'https://youtube.com/@englishwithlucy',
                description: 'İngilizce gramer ve kelime',
                level: 'beginner',
                tags: ['english']
            }
        ]);
    ...
    private addResources(key: string, resources: Resource[]): void {
        this.resources.set(key.toLowerCase(), resources);
    }

    getResources(subject: string, level?: SkillLevel): Resource[] {
        const key = subject.toLowerCase();
        const allResources = this.resources.get(key) || [];
    ...
    searchResources(query: string): Resource[] {
        ...
        this.resources.forEach(resourceList => {
            resourceList.forEach(resource => {
                if (resource.title.toLowerCase().includes(lower) ||
                    resource.description.toLowerCase().includes(lower) ||
                    resource.tags.some(tag => tag.includes(lower))) {
...
class AdvancedStudyAI {
    private state: ConversationState = ConversationState.INITIAL;
    private profile: UserProfile;
    private programData: ProgramData;
    private resourceDB: ResourceDatabase;
    private programManager: StudyProgramManager;  // Program Yöneticisi
    ...
    private readonly POMODORO_WORK = 50;
    private readonly POMODORO_BREAK = 10;

    // AI Conversation History (son 8 mesaj)
    private conversationHistory: Array<{role: 'user' | 'assistant', content: string}> = [];
    private useAI: boolean = true; // AI kullanılsın mı?
    ...
    private async processIntent(intent: AIIntent, message: string): Promise<void> {
        switch (intent) {
    ...
    private handleProgramCreation(message: string): void {
        switch (this.state) {
    ...
    private startProgramCreation(): void {
...
Sadece numarasını veya adını yaz!`);
    ...
    private handleProfileInput(message: string): void {
        const lower = message.toLowerCase();
...
Seviyeni yaz!`);
    ...
    private handleLevelInput(message: string): void {
        const lower = message.toLowerCase();
...
1-16 arası bir sayı. Mesela "6 saat" veya "6"`);
    ...
    private handleHoursInput(message: string): void {
...
1-12 arası bir sayı.`);
    ...
    private handleDurationInput(message: string): void {
...
Bittiğinde **"tamam"** yaz.`);
    ...
    private handleSubjectsInput(message: string): void {
        const lower = message.toLowerCase();
...
Başka konu var mı? Yoksa **"tamam"** yaz.`);
    ...
    private parseSubjects(message: string): Subject[] {
...
**"evet"** veya **"hayır"** yaz.`);
    ...
    private handleBreaksInput(message: string): void {
        const lower = message.toLowerCase();
    ...
    private showSummaryAndGenerate(): void {
        const list = this.profile.subjects.map(s => `• **${s.name}** (${s.priority})`).join('\n');
...
Programını hazırlıyorum... ⚡`);
    ...
    private generateDaySchedule(): TimeSlot[] {
    ...
    private handleResourceRequest(message: string): void {
        const lower = message.toLowerCase();
    ...
    private showResources(title: string, resources: Resource[]): void {
    ...
    private handleTaskAdd(message: string): void {
        const text = message
    ...
    private handleProgressCheck(): void {
...
Detaylı istatistikler için paneldeki **"İstatistikler"** sekmesine bak!`);
    ...
    private handleProgramEdit(message: string): void {
        const lower = message.toLowerCase();
    ...
    private regenerateSchedule(): void {
    ...
    private showScheduleMessage(): void {
...
Hadi senin programını oluşturalım! **"başlayalım"** yaz! 🎯`);
    ...
    private async handleGeneralChat(message: string): Promise<void> {
     ...
     */
    private async askAI(userMessage: string): Promise<void> {
        try {
    ...
    private showProgramPanel(): void {
        this.detailPanel.style.display = 'block';
        this.renderProgramPanel();
    }

    private renderProgramPanel(): void {
    ...
    private showProgramDetails(programId: string): void {
    ...
    private showAddTopicForm(programId: string): void {
    ...
    private toggleTopic(programId: string, topicId: string): void {
        this.programManager.toggleTopicComplete(programId, topicId);
        this.showProgramDetails(programId);
    }

    private deleteProgram(programId: string): void {
...
class StudyProgramManager {
    private programs: StudyProgram[] = [];
    private currentProgram: StudyProgram | null = null;
    private storageKey = 'studyPrograms';

    constructor() {
        this.loadPrograms();
    }

    createProgram(name: string, subject: string, daysPerWeek: number, hoursPerDay: number,
                  resources: {youtube: string[], books: string[]},
                  schedule: {topic: number, practice: number}): StudyProgram {
    ...
    addTopic(programId: string, topicName: string, duration: number): void {
    ...
    toggleTopicComplete(programId: string, topicId: string): void {
        const program = this.programs.find(p => p.id === programId);
        if (program) {
    ...
    getPrograms(): StudyProgram[] {
        return this.programs;
    }

    getCurrentProgram(): StudyProgram | null {
    ...
    getStats(programId: string): {total: number, completed: number, percentage: number} {
        const program = this.programs.find(p => p.id === programId);
...
let app: AdvancedStudyAI;  // Global değişken onclick için
